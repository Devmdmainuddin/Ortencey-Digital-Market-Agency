...
const BlogCard1 = ({ item }) => {
    const { id,image, title, category, totalComments, views, author, description } = item;
    return (
        <div className="w-full  bg-[#f3f3f4] rounded-[20px]">
            ...
            <div className="content py-6 px-[30px]">
                ...
                <Link to={`/blog/${id}`}> <h2 className="mt-4 text-[#1A1A1A] hover:text-[#3661FC] font-nunito text-[25px] font-bold leading-[34px] text-left hover:underline transition-all duration-500">{title}</h2></Link>
                ...
                <div className="flex gap-[60px] items-center mt-6">
                  ...
                  <Link to={`/blog/${id}`}><button className="flex items-center gap-6 text-[#6B6161] font-dm-sans text-[18px] font-normal leading-[26px] relative">Read More <FaArrowRight className="text-xl absolute right-5" /> <div className="w-[33px] h-[33px] rounded-full bg-gradient-to-r from-[#fc8e57] to-[#FFAA52]"></div></button> </Link>  
                </div>
